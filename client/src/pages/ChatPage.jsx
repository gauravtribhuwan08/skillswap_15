...
import { BsCameraVideoFill } from 'react-icons/bs'; // Importing a video camera icon
...
const ChatPage = () => {
  ...
  useEffect(() => {
    if (!sessionId) {
      // No active session cleanly returns
      return;
    }
    ...
    socketIo.on('callUser', (data) => {
      ...
      setIncomingCall(data); // data contains from, name, signal
    });
            ...
            {selectedConnection && (
              <>
                ...
                <div className="flex flex-wrap justify-center items-center gap-4 mt-4">
                  ...
                  {/* Video Call Button */}
                  {!isChatBlocked && !isSessionCompletedOrCanceled && socket && (
                    <button
                      onClick={() => {
                        setIncomingCall(null);
                        setIsVideoCallOpen(true);
                      }}
                      className="flex items-center gap-2 py-2 px-4 bg-purple-600 text-white font-semibold rounded-lg shadow-lg hover:bg-purple-700 transition duration-200"
                    >
                ...
                {isVideoCallOpen && (
                  <VideoCallModal
                    ...
                    otherUser={selectedConnection.userId1._id === loggedInUser._id ? selectedConnection.userId2 : selectedConnection.userId1}
